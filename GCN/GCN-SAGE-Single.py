...
if __name__ == '__main__':
    ...
    for layer in range(args.n_layers + 1):
        for nodes in range(g.number_of_nodes()):
            if layer == 0:
                ...
                Batch_sampling_method = np.append(Batch_sampling_method, 10)
    ...
    dataframe.to_csv("/home/fahao/Py_code/results/GCN-Pubmed(8)/single/acc_gcn_SAGE.csv",header = False,index=False,sep=',')
    dataframes.to_csv("/home/fahao/Py_code/results/GCN-Pubmed(8)/single/acc_gcn_SAGE(round).csv",header = False,index=False,sep=',')
